""" utility functions regarding logging, random seeds, etc. """
import os
...
def init_logger(level: str = "INFO", to_file: bool = False, verbose: bool = True, log_file_notion: str = None):
    """initialize logger
    ...
    try:
        ...
        logger.add(lambda msg: tqdm.write(msg, end=""), colorize=True, format=fmt, level=level)
        tqdm_enabled = True
    except ImportError:
        logger.add(lambda msg: print(msg, end=""), colorize=True, format=fmt, level=level)
...
def set_random_seeds(seed: int = 42):  # sourcery skip: extract-method, use-contextlib-suppress
